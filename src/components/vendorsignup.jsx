      ...
      >
        ...
        >
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="email" id="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="Enter your email" className={`w-full bg-gray-50 border rounded-lg px-10 py-3 text-gray-800 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all ${errors.email ? 'border-red-500 bg-red-50' : 'border-gray-200'}`} disabled={isLoading} />
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type={showPassword ? 'text' : 'password'} id="password" name="password" value={formData.password} onChange={handleInputChange} placeholder="Enter your password" className={`w-full bg-gray-50 border rounded-lg px-10 py-3 pr-12 text-gray-800 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all ${errors.password ? 'border-red-500 bg-red-50' : 'border-gray-200'}`} disabled={isLoading} />
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-orange-600 transition-colors cursor-pointer" aria-label={showPassword ? 'Hide password' : 'Show password'} disabled={isLoading}>
                        {showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                      </button>
                    </div>
                    {errors.password && (<p className="text-red-600 text-sm mt-1" role="alert">{errors.password}</p>)}
                  </div>
                  <button type="button" onClick={handleAuth} disabled={isLoading} className="w-full bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white font-semibold py-3 px-4 rounded-lg transition-all duration-200 hover:shadow-lg hover:scale-[1.02] disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 cursor-pointer">
                    {isLoading ? (<div className="flex items-center justify-center"><div className="spinner mr-2"></div>Signing In...</div>) : ('Sign In')}
                  </button>
                  <div className="text-center pt-2">
                    <button type="button" className="text-sm text-orange-600 hover:text-orange-800 transition-colors cursor-pointer" disabled={isLoading}>Forgot Password?</button>
                  </div>
                </div>
              </>
            ) : (
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="text" id="firstName" name="firstName" value={formData.firstName} onChange={handleInputChange} placeholder="Enter first name" className={`w-full bg-gray-50 border rounded-lg px-10 py-3 text-gray-800 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all ${errors.firstName ? 'border-red-500 bg-red-50' : 'border-gray-200'}`} disabled={isLoading} />
                      </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <User className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
                        <input type="text" id="lastName" name="lastName" value={formData.lastName} onChange={handleInputChange} placeholder="Enter last name" className={`w-full bg-gray-50 border rounded-lg px-10 py-3 text-gray-800 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all ${errors.lastName ? 'border-red-500 bg-red-50' : 'border-gray-200'}`} disabled={isLoading} />
                      </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative">
                        <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
                        <input type={showPassword ? 'text' : 'password'} id="password" name="password" value={formData.password} onChange={handleInputChange} placeholder="Create password" className={`w-full bg-gray-50 border rounded-lg px-10 py-3 pr-12 text-gray-800 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all ${errors.password ? 'border-red-500 bg-red-50' : 'border-gray-200'}`} disabled={isLoading} />
                        <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-orange-600 transition-colors cursor-pointer" aria-label={showPassword ? 'Hide password' : 'Show password'} disabled={isLoading}>
                          {showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                        </button>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
                        <input type={showConfirmPassword ? 'text' : 'password'} id="confirmPassword" name="confirmPassword" value={formData.confirmPassword} onChange={handleInputChange} placeholder="Confirm password" className={`w-full bg-gray-50 border rounded-lg px-10 py-3 pr-12 text-gray-800 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all ${errors.confirmPassword ? 'border-red-500 bg-red-50' : 'border-gray-200'}`} disabled={isLoading} />
                        <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-orange-600 transition-colors cursor-pointer" aria-label={showConfirmPassword ? 'Hide password' : 'Show password'} disabled={isLoading}>
                  ...
                  <div>
                    <label className="flex items-start cursor-pointer group">
                      <input type="checkbox" name="agreeTerms" checked={formData.agreeTerms} onChange={handleInputChange} className="w-4 h-4 text-orange-600 bg-white border-2 border-gray-300 rounded focus:ring-orange-500 focus:ring-2 mt-0.5 transition-colors cursor-pointer" disabled={isLoading} />
                      <span className="text-sm text-gray-700 ml-3 leading-relaxed">
                        I agree to the <button type="button" className="text-orange-600 hover:text-orange-800 underline font-medium transition-colors cursor-pointer" onClick={() => handleTermsClick('terms')} disabled={isLoading}>Terms and Conditions</button> and <button type="button" className="text-orange-600 hover:text-orange-800 underline font-medium transition-colors cursor-pointer" onClick={() => handleTermsClick('privacy')} disabled={isLoading}>Privacy Policy</button>
                      </span>
                    </label>
          ...
          <div className="text-center pt-4 border-t border-gray-200">
            {isLoginView ? (
              <p className="text-gray-600 text-sm">Don't have an account? <button onClick={handleViewToggle} className="text-orange-600 hover:text-orange-800 font-semibold transition-colors underline cursor-pointer" disabled={isLoading}>Create an Account</button></p>
            ) : (
              <p className="text-gray-600 text-sm">Already have an account? <button onClick={handleViewToggle} className="text-orange-600 hover:text-orange-800 font-semibold transition-colors underline cursor-pointer" disabled={isLoading}>Sign In</button></p>
            )}
          </div>
        </div>
      </div>
